...
class TinyImageNetCNN(nn.Module):
    def __init__(self, num_classes=200):
        ...
        # We register these as buffers so they move to GPU automatically with the model
        # but are not treated as trainable weights.
    ...
    def forward(self, x):
        # 1. Normalize inputs (x - mean) / std
        # This allows the input 'x' to be in range [0, 1]
        x_norm = (x - self.mean) / self.std
        
        # 2. Standard Forward Pass
        x = self.block1(x_norm)
        x = self.block2(x)
        x = self.block3(x)
        x = self.block4(x)
        x = self.flatten(x)
...
def setup_model_and_image():
    ...
    # Check if user put image in main folder or images subfolder
    ...
    # strict=False allows us to load the weights even though 'mean' and 'std' 
    # are in the class but not in the saved file.
    model.load_state_dict(state_dict, strict=False)
    model.eval()

    # 3. Load Image (Raw [0, 1] Tensor)
    # We do NOT use transforms.Normalize here because the model does it internally now.
    preprocess = transforms.Compose([
        transforms.Resize((64, 64)), # Safety check
    ...
    # 5. Create Target Vector
    c_vector = torch.zeros((1, 200), device=device)
