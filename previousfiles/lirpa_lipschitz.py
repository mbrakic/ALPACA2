...
def example_jacobian(model_ori, x0, eps, bound_opts, device):
...
def example_local_lipschitz(model_ori, x0, eps, bound_opts, device):
...
def compute_jacobians(model_ori, x0, eps, bound_opts=None, device='cpu'):
    ...
    results[0] = example_jacobian(model_ori, x0, eps, bound_opts, device)
    results[1] = example_local_lipschitz(model_ori, x0, eps, bound_opts, device)
...
if __name__ == '__main__':
    ...
    compute_jacobians(model_ori, x0, eps, device=device)