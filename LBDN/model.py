...
class fcSimple(nn.Module):
    def __init__(self, config):
        ...
        g = self.gamma ** 0.5
        if self.img_size == 32:
            # Input features for a flattened 3x32x32 image
            input_features = self.in_channels * 32 * 32 
            
            # This is the main body of the network
            layer = [
                nn.Flatten(),
                
                # --- Hidden Layer 1 ---
                SandwichFc(input_features, 1024, scale=g),
                nn.Dropout(0.5),
                
                # --- Hidden Layer 2 ---
                SandwichFc(1024, 512, scale=g),
                nn.Dropout(0.3),
                
                # --- Hidden Layer 3 ---
                # Continue tapering to a reasonable size for the classifier
                SandwichFc(512, 256, scale=g),
            ]

        # Adapt the final layers to the output of the main body (256 features)
        if self.LLN:
            # The input to this block is now 256, not 2048
            layer.append(SandwichFc(256, 128, scale=g))
            layer.append(LinearNormalized(128, self.num_classes))
        else:
            # The input to this block is now 256, not 2048
            layer.append(SandwichFc(256, 128))
            layer.append(SandwichLin(128, self.num_classes, scale=g))
